...
function datagrid_add_row(table, field_id, formurl) {
	datagrid_show_form(field_id, formurl, function(rowdata, raw) {
		...
		//jq('#' + field_id + '_editrow').removeClass("disabled");
		//jq('#' + field_id + '_deleterow').removeClass("disabled");
	...
	if (row) {
		...
		table.fnDeleteRow(row, undefined, true);
                if (table.fnGetData().length==0){
                    jq('#' + field_id + '_editrow').addClass("disabled");
		    jq('#' + field_id + '_deleterow').addClass("disabled");
                }
	}
	else {
