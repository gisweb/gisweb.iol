...
jQuery(document).ready(function () {
     ...
     jQuery("[data-plugin='geocode']").each(function(){
          ...
          if(jQuery('#'+options.icon).length) options.icon = jQuery('#'+options.icon).attr('value');
          if(jQuery(this).data('iconPath')) options.icon = jQuery(this).data('iconPath') + options.icon;
...
function updateMapxx(data,mapId){
...
function geocode_actions(){
    ...
    if(jQuery('#'+options.icon).length) options.icon = jQuery('#'+options.icon).attr('value');
    if(jQuery(this).data('iconPath')) options.icon = jQuery(this).data('iconPath') + options.icon;
    if(jQuery(this).data('baseUrl')) options.baseUrl = jQuery(this).data('baseUrl');
...
function geocode_catasto(options){
	...
	jQuery.ajax({
                'url':options.baseUrl + '/services/xSuggest',
		'type':'POST',
		'data':params,
		'dataType':'JSON',
		'success':function(data, textStatus, jqXHR){
